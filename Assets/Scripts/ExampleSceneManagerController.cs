...
public class ExampleSceneManagerController : MonoBehaviour
{
	...
	{
		WSANativeFilePicker.PickSingleFile("Select", WSAPickerViewMode.Thumbnail, WSAPickerLocationId.PicturesLibrary, new string[2]
		{
			".png",
			".jpg"
	...
	public void ShowFileSavePicker()
	{
		...
		{
			new KeyValuePair<string, IList<string>>("Text Files", new List<string>
			{
				".txt"
			})
		}, delegate(CI.WSANative.Pickers.WSAStorageFile result)
	...
	public void FacebookLogin()
	{
		...
		WSANativeFacebook.Login(new List<string>
		{
			"public_profile",
			"email",
			"user_birthday"
		}, delegate(WSAFacebookLoginResult result)
		{
			if (result.Success)
			{
				WSANativeFacebook.GetUserDetails(delegate(WSAFacebookResponse<WSAFacebookUser> response)
				{
					if (response.Success)
					{
						WSAFacebookUser datum = response.Data;
					}
				});
			}
		});
